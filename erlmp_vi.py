...
for trial in range(1, num_trials + 1):
    ...
    class UrnAgent:
      ...
      def visualize_urn(agent, role, gen = None, output_dir="urn", title="Urn Contents"):
        if gen is not None:
            title = f"{title} (Generation {gen})"

            num_obs = agent.num_obs  # 観測の数
            num_choices = agent.num_choices  # 選択肢の数

            # データ収集
            current_balls = np.array([urn for urn in agent.urn_balls])  # 現在の壺のボールの数
            genetic_bias = agent.bias_gene.values  # 遺伝的バイアス

            # プロットの準備
            fig, ax = plt.subplots(figsize=(12, 6))
            bar_width = 0.35  # 棒の幅
            x = np.arange(num_obs * num_choices)  # 棒グラフのX軸

            # 現在のボール数
            current_balls_flat = current_balls.flatten()
            ax.bar(x - bar_width / 2, current_balls_flat, bar_width, label="Current Balls")

            # 遺伝的バイアス
            genetic_bias_flat = genetic_bias.flatten()
            ax.bar(x + bar_width / 2, genetic_bias_flat, bar_width, label="Genetic Bias")

            # ラベルとタイトル
            ax.set_xlabel(f"Observations & Choices ({role})")
            ax.set_ylabel("Number of Balls")
            ax.set_title(title)

            #凡例の調整
            ax.legend(bbox_to_anchor=(0.5, -0.15), loc='upper center', ncol=2, fontsize=10)

            # X軸のカスタマイズ
            x_labels = [f"{obs}->{choice}" for obs in range(num_obs) for choice in range(num_choices)]
            ax.set_xticks(x)
            ...
            # ファイル名の生成
            filename = f"gen{gen}_{title.replace(' ', '_')}.png"
    ...
    for gen in range(num_gen): # 生まれた瞬間の成功率をとって、報酬0の場合（遺伝のみ）と学習する場合で比較
        ...
        if gen % 100 == 0 or gen == 999:
            ...
            UrnAgent.visualize_urn(best_sender, "sender", gen, title="Best Sender's Urn Contents")
            best_receiver = best_creature.agents["receiver"]
            UrnAgent.visualize_urn(best_receiver, "receiver", gen, title="Best Receiver's Urn Contents")
